...
function inspectInstalledPlugin(pluginName, opts = {}) {
  ...
  const configPath = candidateConfigPaths.find((candidate) => fs2.existsSync(candidate)) ?? candidateConfigPaths[0];
  ...
  const cachePathExists = Boolean(cachedPluginPath && fs2.existsSync(cachedPluginPath));
...
function resolveCopilotConfigPath(homeDir, explicitConfigPath) {
  ...
  if (fs3.existsSync(rawCopilotConfigPath)) return rawCopilotConfigPath;
...
function findLegacyHookPluginConflicts(opts = {}) {
  ...
  for (const entry of pluginEntries) {
    ...
    if (identityReasons.length > 0 && (!hookManifestPath || !fs3.existsSync(hookManifestPath))) {
      conflicts.push({
        pluginName,
        configPath,
        cachePath,
    ...
    if (hookManifestPath && fs3.existsSync(hookManifestPath)) {
...
function readCopilotRootModel(configPath) {
  if (!configPath || !fs4.existsSync(configPath)) return normalizeRootModel(null);
  try {
    const parsed = parseJsonWithComments(fs4.readFileSync(configPath, "utf8"));
...
function buildCandidates(kind, id, options) {
  ...
  return [
    { layer: "user-level-profile", path: userPath, exists: fs4.existsSync(userPath), ...readFrontmatterMetadata(userPath) },
    { layer: "project-level", path: projectPath, exists: fs4.existsSync(projectPath), ...readFrontmatterMetadata(projectPath) },
    {
      layer: "plugin-installed",
      path: pluginPath,
      exists: options.pluginCachePath ? fs4.existsSync(pluginPath) : false,
...
function compareMirrors(expectedRoot, actualRoot, label) {
  assert.ok(fs5.existsSync(actualRoot), `${label} is missing: ${actualRoot}`);
...
${globalRepairHint(label)}`);
  for (const file of expected) {
    const expectedContent = fs5.readFileSync(path6.join(expectedRoot, file), "utf8");
    const actualContent = fs5.readFileSync(path6.join(actualRoot, file), "utf8");
...
${globalRepairHint(label)}`);
  for (const file of expected) {
    const expectedContent = renderRuntimeAgentContent(fs5.readFileSync(path6.join(expectedSourceRoot, file), "utf8"), {
...
function createGitHubRemoteWorkspace() {
  const workspace = fs5.mkdtempSync(path6.join(os3.tmpdir(), "xgc-validate-github-workspace-"));
...
function main() {
  ...
  assert.ok(fs5.existsSync(paths.profileHome), `X for GitHub Copilot profile home is missing: ${paths.profileHome}`);
  assert.ok(fs5.existsSync(paths.configHome), `X for GitHub Copilot config home is missing: ${paths.configHome}`);
  assert.ok(fs5.existsSync(paths.profileConfigPath), `profile config is missing: ${paths.profileConfigPath}`);
  assert.ok(fs5.existsSync(paths.profileMcpConfigPath), `profile MCP config is missing: ${paths.profileMcpConfigPath}`);
  assert.ok(fs5.existsSync(paths.profileLspConfigPath), `profile LSP config is missing: ${paths.profileLspConfigPath}`);
  assert.ok(fs5.existsSync(paths.profileHookScriptsDir), `profile hook scripts are missing: ${paths.profileHookScriptsDir}`);
  assert.ok(fs5.existsSync(paths.shellShimPath), `installed X for GitHub Copilot shell shim is missing: ${paths.shellShimPath}`);
  assertProfileEnvHomeTruth(paths);
  assert.ok(fs5.existsSync(path6.join(paths.profileHookScriptsDir, "session-start.sh")), "session-start hook script is missing from X for GitHub Copilot config home");
  assert.ok(fs5.existsSync(path6.join(paths.profileHookScriptsDir, "common.sh")), "hook common helper is missing from X for GitHub Copilot config home");
  ...
  const profileLspConfig = JSON.parse(fs5.readFileSync(paths.profileLspConfigPath, "utf8"));
  assert.ok(
  ...
  const profileConfig = readJsonIfExists(paths.profileConfigPath) ?? {};
  ...
  assert.equal(
    fs5.readFileSync(paths.shellShimPath, "utf8"),
    fs5.readFileSync(path6.join(args.repoRoot, "scripts", "xgc-shell.sh"), "utf8"),
...
`);
  ...
  const fakeRoot = fs5.mkdtempSync(path6.join(os3.tmpdir(), "xgc-shell-validate-"));
  ...
  const shellEnvText = fs5.existsSync(paths.shellEnvPath) ? fs5.readFileSync(paths.shellEnvPath, "utf8") : "";
